...
class TestProductType(TestCase):
    def setUp(self):
        ...
        self.past_price = PriceFactory.create(
            product_type=self.product_type, start_date=date(2020, 1, 1)
        )
        self.present_price = PriceFactory.create(
            product_type=self.product_type, start_date=date(2024, 1, 1)
        )
        self.future_price = PriceFactory.create(
            product_type=self.product_type, start_date=date(2025, 1, 1)
        )
