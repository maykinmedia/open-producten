...
from openproduct.setup_configuration.steps import ReferentielijstenConfigurationStep
...
class ReferentielijstenConfigurationStepTests(TestCase):
    ...
    def test_happy_flow(self):
        service = ServiceFactory(
            slug="referentielijsten-api-test",
            api_root="http://test.nl/api/v1",
        )
    ...
    def test_idempotency(self):
        service = ServiceFactory(
            slug="referentielijsten-api-idempotent",
            api_root="http://idempotent.nl/api/v1",
        )
        ...
        execute_single_step(
            ...
            yaml_source=test_file_path,
        )

        config = ReferentielijstenConfig.get_solo()
        self.assertTrue(config.enabled)
        self.assertEqual(config.service, service)
        self.assertEqual(config.kanalen_tabel_code, "KANAAL_IDEMPOTENT")
